...
import com.abishek.android.apps.exposurenotification.common.StringUtils;
...
public class ExposureBottomSheetFragment extends BottomSheetDialogFragment {
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    if (getArguments() != null) {
      ...
      time = getFormattedInfoText(receivedTimestampMs);
      infoStatus.setText(time);
    }
    final String timestamp = time;
    learnMore.setOnClickListener((v) -> learnMoreClicked(timestamp));
    done.setOnClickListener((v) -> doneClicked());
  }

  private void learnMoreClicked(String timestamp) {
    Intent intent = new Intent(requireContext(), ExposureLearnMoreActivity.class);
